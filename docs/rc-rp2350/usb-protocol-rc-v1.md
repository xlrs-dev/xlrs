...
```text
rc.v1 hello seq=42
rc.v1 ok seq=42 role=rc_handset fw=0.1 caps=state,config,binding,tx_proxy
```
...
Writes a named RC handset setting. Requires `field` and `value`. The WebUI uses
fields such as `cal.min.0`, `axis.invert.0`, `channel.trim.0`,
`filter.low_pass`, and compact `filter=<oversample,low_pass,temporal,highpass>`.

```text
rc.v1 set_config seq=11 field=binding_phrase value=Bench%20Rig
rc.v1 ok seq=11 field=binding_phrase
```

### apply, save, reset_defaults

WebUI utility commands for staging and persisting RC-local config.
...
Returns a current handset state snapshot for the WebUI. The integrated RC
firmware currently returns a compact, percent-encoded `state=` value inside an
`ok` response. Host tools should also tolerate future event-style `rc.v1 state`
lines.

```text
rc.v1 state seq=12
rc.v1 ok seq=12 state=state%20tx%3Dpresent%20tx_status%3Dvalid%20rate%3DL100%20uid%3Dmatch%20link%3Dvalid%20lq%3D98%20rssi%3D-47%20snr%3D9%20channels%3Dvalid%20ch%3D992%2C992%2C172%2C992%20faults%3Dnone
...
| Write setting | `set_config field=<name> value=<encoded>` |
| Apply staged config | `apply` |
| Persist active config | `save` |
...
Those commands are kept in the protocol so the WebUI and tests can stay aligned,
but the integrated firmware may return `unsupported_command` until each runtime
handler is added.
...
rc.v1 set_config seq=7 field=binding_phrase value=Bench%20Rig
