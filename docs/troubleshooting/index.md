...
5. A nonzero `missedDeadlines` means the RF task skipped stale timer backlog and
   fast-forwarded to the current scheduler tick. Stale RF slots are not replayed. If `fhss`
   and `exp` stay matched afterward, the scheduler recovered; if they diverge or
   `missedDeadlines` climbs continuously, look for blocking work on core 1.
