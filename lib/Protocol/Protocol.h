...
#define FRAME_SIZE (CHANNEL_DATA_SIZE + SEQUENCE_SIZE + HMAC_SIZE)  // 22 bytes total
...
public:
    ...
    static void encodeFrame(uint16_t channels[NUM_CHANNELS], uint16_t sequence, Security* security, uint8_t* frame);
    
    // Decode binary frame into channel values and verify security
    // Returns true if frame is valid and security checks pass
    static bool decodeFrame(const uint8_t* frame, uint16_t channels[NUM_CHANNELS], uint16_t* sequence, Security* security, uint16_t* lastSequence);
