...
|---|---|---|---|---|
| **P0** | **Binding UID consistency** | **[COMPLETE]** Derives FNV-1a UIDs consistently on TX/RX from EEPROM, calculates 16-bit hardware sync words, accepts UART commands, and reboots. | None. Maintain this unified pipeline as new modulation rates are introduced. | Removes split identity behavior. TX/RX identity is deterministic, cheap, host-testable, and shared by FHSS + sync-word derivation. |
...
| **P0** | **OTA packet shrink** | Current RC path sends 8 channels at 11-bit precision (12-byte payload). Sync is 10 bytes. | Add compact RC modes: primary controls plus switch multiplexing in a small frame (e.g. 8-byte target), with a larger/full-resolution mode only when needed. | Reduces airtime, improves link budget, enables higher packet rates, and avoids spending bytes on controls that rarely change. |
| **P0** | **Real FHSS region tables** | Has UID-seeded FHSS and a simple 20-channel 2.4 GHz table. | Add production regional/domain tables, channel spacing rules, dwell accounting, region enforcement, and CE LBT support. | FHSS becomes both interference-resistant and compliance-ready instead of only demonstrating that hopping works. |
...
| **P1** | **Reliable telemetry transport** | Sends downlink link statistics. Does not yet have robust fragmented data transport for MSP/config/sensor telemetry. | Add a reliable packetized telemetry/data layer with ack, retry, sequencing, and bounded bandwidth use (e.g. `StubbornSender` / `StubbornReceiver` queue). | Enables configuration, diagnostics, and telemetry without corrupting RC timing or relying on best-effort single packets. |
...
| **P1** | **Config safety** | **[COMPLETE]** Formally partitioned EEPROM (RfConfig at base 0, Security at 120, RCConfig at 200), added magic/checksum validation, and auto-rollback defaults. | Define migrations, rollback/default behavior, and write lockout rules during RF-critical operation. | Prevents field-bricking and cross-config corruption as schemas evolve. |
...
| **P1** | **Link-state machine hardening** | Has `Connecting`, `Connected`, `Failsafe`, a lock flag, and reacquisition behavior. | Add explicit disconnected/tentative/locked substates and gate RC output until timing and model identity are stable. | Prevents premature RC output during partial sync, wrong model, or unstable timing conditions. |
| **P1** | **Link diagnostics** | **[COMPLETE]** Visual FastLED blinking UX for hardware fault/config error and handset UART connection state mapping. | Expose CRC errors, telemetry misses, PFD offset, FHSS index, power, lock state, hardware health, and recovery history. | Makes flight testing and field support practical. Silent failure modes become observable. |
...
| **P3** | **Remove legacy contradictions** | New XLRS core coexists with legacy `Security`, `Protocol`, and older SX128x paths. | Cut over or isolate legacy paths once the new core reaches parity. Remove duplicated identity, crypto, and packet-format logic. | Production firmware should have one source of truth. This eliminates misleading claims and split behavior. |
...
4.  **[P0] Shrink the RC OTA frame**: Write an alternative compact 8-byte primary controls OTA frame to maximize range and receiver sensitivity.
5.  **[P1] Add reliable telemetry/data transport**: Design a robust `StubbornSender` / `StubbornReceiver` queue to handle MSP and MAVLink bidirectional telemetry.
