...
5.  **[COMPLETE] Advanced User-Visible State UX**: 
    *   **Receiver status indicator**: Programmed distinct link/error states for the RX module status output.
    *   **TX UART State Mapping**: Mapped `xlrs::LinkState` to compact UART connection codes (`0` to `4`) for controller-side status consumers.
...
| **P0** | **Hardware timing validation** | Has `HwTimer`, `RfScheduler`, `Pfd`, async PHY hooks, and native timing tests. Native SX1280 turnaround compensation is modeled. | Measure real scheduler jitter, RX PFD lock time, drift tolerance, missed deadlines, hop-boundary timing, and telemetry turnaround on physical TX/RX hardware. | Turns architecture into evidence. Timing is the foundation for FHSS, collision-free telemetry, latency, and failsafe timing. |
| **P0** | **SX1280 native driver hardening** | Has `Sx1280NativePhy` with async TX/RX, command timeout checks, physical `resetRadio()` recovery, and `_hardwareError` tracking. **Audited register-by-register against SX1280 datasheet Rev 3.3**: opcodes, frequency, power, LoRa/FLRC mod+packet params, IRQ masks, and RSSI/SNR decode all confirmed. Fixed: FLRC 0.65 Mb/s bitrate byte (`0x8A`→`0x86`, Table 14-31, hit the D250 rate), AutoFS now uses the documented `SetAutoFS` command (0x9E) instead of an undocumented register write, RxGain high-sensitivity now read/modify/write, TX power clamped to the 0..31 register range, and a §16.4 forbidden FLRC sync-word guard. | Bench-validate the corrected values on hardware: AutoFS turnaround timing, high-sensitivity RSSI floor, CRC-error rejection, power commit, and recovery. Confirm the §16.4 sync-word remap on air. The driver path is not covered by the native suite, so this needs a radio. | Owning the driver gives deterministic SPI timing and lower overhead while preserving the `IRadioPhy` boundary. |
...
| **P1** | **Link diagnostics** | **[PARTIAL]** RX status output, TX UART connection state mapping, and internal stats for telemetry misses, FHSS index, current TX power, queue drops, and PHY recovery history. | Expose CRC errors, PFD offset, lock state, hardware health, and recovery history through CRSF/UART/debug streams. | Makes flight testing and field support practical. Silent failure modes become observable. |
| **P2** | **CRSF ecosystem support** | RX emits RC channels and link statistics; TX sends basic telemetry/status to the controller-side UART peer. | Add CRSF device info, parameter routing, richer telemetry, config commands, and optional MSP/MAVLink bridging where product requirements need them. | Integrates cleanly with FCs and tooling without pushing app complexity into the RF core. |
...
| **P2** | **CI and native tests** | **[COMPLETE]** A 35-test native suite under `test/` runs off-device via CMake/CTest (`scripts/test.sh`, Unity fetched at configure time), covering UID/FHSS, OTA pack/CRC, AEAD crypto, PFD timing/lock, link connect/failsafe, bind mismatch, rate switching, corrupt-frame rejection, PHY recovery including repeated-retry failures, and a long simulated soak. ARM firmware builds via the top-level Pico SDK CMake. | Add tests for integrated telemetry-loss power boost, UID migration, and EEPROM/RfConfig schema validation. Wire `scripts/test.sh` into CI. | Keeps the clean architecture honest as complexity grows. Fast tests catch regressions before bench time. |
...
| **P3** | **Remove legacy contradictions** | **[COMPLETE]** Production firmware now has one XLRS link core. Legacy `Security`, `Protocol`, `SX128xLink`, legacy SX128x mains, stale build environments, and controller-only code were removed. | Keep future experimental paths behind explicit new names and avoid duplicated identity, crypto, or OTA packet logic. | Production firmware has one source of truth, eliminating misleading claims and split behavior. |
