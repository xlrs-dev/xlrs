...
#define RC_ELRS_PARAM_DISCOVERY_MS 3000UL
#endif
...
void receiveUARTMessages() {
    crsfSerial.loop();
    if (tx_type == TX_TYPE_ELRS && crsfSerial.isLinkUp()) {
        telemetryValid = true;
...
void handleButtons() {
    ...
    if (enterPressed && !enterButtonPressed && (millis() - lastEnterPress) > BUTTON_DEBOUNCE_MS) {
        ...
        if (menu_screen == MENU_MAIN) {
            ...
            if (menu_cursor == 0) {
                if (!elrs_discover_params(true, false, RC_ELRS_PARAM_DISCOVERY_MS))
...
void ping_tx_device(void) {
    ...
    crsfSerial.queueExtendedPacket(CRSF_ADDRESS_CRSF_TRANSMITTER,
        CRSF_ADDRESS_ELRS_LUA, CRSF_FRAMETYPE_DEVICE_PING, nullptr, 0);
...
static void elrs_request_param_chunk(uint8_t fieldId, uint8_t chunkIndex) {
    ...
    crsfSerial.queueExtendedPacket(CRSF_ADDRESS_CRSF_TRANSMITTER, CRSF_ADDRESS_ELRS_LUA, CRSF_FRAMETYPE_PARAMETER_READ, req, 2);
    flush_crsf_tx_queue();
}
...
static void onCrsfParameterEntry(uint8_t fieldId, uint8_t paramType, uint8_t chunksRemaining, const char *label, const uint8_t *value, uint8_t valueLen) {
    ...
    auto scan_io = [&]() {
        if (fieldId != elrs_param_chunk_next || !elrs_param_read_in_progress)
            return false;
        elrs_last_param_entry_ms = millis();
        return true;
    };
    ...
    if (labeled && paramType == 0x0A) {
        ...
        if (strstr(lab, "Bind") != nullptr && strstr(lab, "hrase") == nullptr) {
        ...
        if (elrs_wifi_cmd_field_id == ELRS_PARAM_FIELD_NONE &&
            (strstr(lab, "WiFi") || strstr(lab, "wifi") || strstr(lab, "Wifi"))) {
    ...
    } else if (fieldId < ELRS_PARAM_MAX_CHUNKS) {
        elrs_param_chunk_next = (uint8_t)(fieldId + 1);
        elrs_param_chunk_index = 0;
...
static bool elrs_write_param_string(uint8_t fieldId, const char *str) {
    ...
    buf[1 + len] = '\0';
    crsfSerial.queueExtendedPacket(CRSF_ADDRESS_CRSF_TRANSMITTER, CRSF_ADDRESS_ELRS_LUA, CRSF_FRAMETYPE_PARAMETER_WRITE, buf, (uint8_t)(1 + len + 1));
    return true;
}

static bool elrs_write_param_byte(uint8_t fieldId, uint8_t val) {
    uint8_t buf[2] = { fieldId, val };
    crsfSerial.queueExtendedPacket(CRSF_ADDRESS_CRSF_TRANSMITTER, CRSF_ADDRESS_ELRS_LUA, CRSF_FRAMETYPE_PARAMETER_WRITE, buf, 2);
    return true;
}
...
static bool attempt_handset_uart(uint32_t baud, bool invert, unsigned dwellMs) {
    ...
    crsfSerial.onExtendedFrame = onCrsfExtendedFrame;
    crsfSerial.onParameterEntry = onCrsfParameterEntry;
    crsfSerial.queueExtendedPacket(CRSF_ADDRESS_CRSF_TRANSMITTER, CRSF_ADDRESS_ELRS_LUA, CRSF_FRAMETYPE_DEVICE_PING, nullptr, 0);
    flush_crsf_tx_queue();
...
static void elrs_simpletx_bind_burst(void) {
    uint8_t frm[8];
    simpletx_crsf::prepareCmdPacket(frm, RC_SIMPLETX_ELRS_CMD_BIND, 1);
    for (unsigned n = 0; n < 5; n++)
        crsfSerial.queueCrsfRawTx(frm, sizeof(frm));
    flush_crsf_tx_queue();
}
...
static void elrs_simpletx_wifi_burst(void) {
    uint8_t frm[8];
    // kkbin LUA indices: 14 = Enable WiFi (TX web UI), 15 = Rx WiFi; send both so TX hotspots come up on mixed ELRS builds.
    simpletx_crsf::prepareCmdPacket(frm, 0x0E, 1);
    for (unsigned n = 0; n < 5; n++)
        crsfSerial.queueCrsfRawTx(frm, sizeof(frm));
    simpletx_crsf::prepareCmdPacket(frm, RC_SIMPLETX_ELRS_CMD_WIFI, 1);
    for (unsigned n = 0; n < 5; n++)
        crsfSerial.queueCrsfRawTx(frm, sizeof(frm));
    flush_crsf_tx_queue();
}
...
static bool rc_proto_set_binding_phrase_tx(const char* phrase, uint8_t len) {
    ...
    if (tx_type == TX_TYPE_ELRS && elrs_role == ELRS_ROLE_TX) {
        if (elrs_phrase_field_id == ELRS_PARAM_FIELD_NONE && !elrs_discover_params(true, false, RC_ELRS_PARAM_DISCOVERY_MS))
            return false;
...
static bool rc_proto_enter_pairing_mode(void) {
    ...
    elrs_send_rx_bind_crsf_command();
    elrs_simpletx_bind_burst();
    Serial.println("[RC] ELRS bind: CRSF COMMAND + SimpleTX SETTINGS bursts sent");

    if (elrs_bind_cmd_field_id == ELRS_PARAM_FIELD_NONE)
        (void)elrs_discover_params(false, true, RC_ELRS_PARAM_DISCOVERY_MS);
    if (elrs_bind_cmd_field_id != ELRS_PARAM_FIELD_NONE) {
        if (elrs_write_param_byte(elrs_bind_cmd_field_id, 0x01))
            Serial.println("[RC] ELRS bind Lua command click sent");
    }
    return true;
}

static bool rc_proto_enter_wifi_mode(void) {
    if (tx_type != TX_TYPE_ELRS || elrs_role != ELRS_ROLE_TX) return false;
    if (elrs_wifi_cmd_field_id == ELRS_PARAM_FIELD_NONE) {
        ...
        elrs_discover_params(true, true, RC_ELRS_PARAM_DISCOVERY_MS);
    }
    if (elrs_wifi_cmd_field_id != ELRS_PARAM_FIELD_NONE) {
        bool sent = elrs_write_param_byte(elrs_wifi_cmd_field_id, 0x01);  // CLICK = enable WiFi
        if (sent) {
            Serial.println("[RC] ELRS WiFi mode command sent");
            return true;
        }
    }
    elrs_simpletx_wifi_burst();
    Serial.println("[RC] ELRS WiFi: SimpleTX SETTINGS burst sent (fallback)");
    return true;
}
